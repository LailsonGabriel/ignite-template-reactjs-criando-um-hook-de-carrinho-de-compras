...
export function CartProvider({ children }: CartProviderProps): JSX.Element {
  ...
  const addProduct = async (productId: number) => {
    try {
      ...
      const currentAmout = productInCart ? productInCart.amount : 0;
  ...
  const removeProduct = (productId: number) => {
    try {
      const newCart = [...cart];
      const resultRemoved = newCart.filter((product) => product.id !== productId);
      setCart(resultRemoved);
      localStorage.setItem('@RocketShoes:cart', JSON.stringify(resultRemoved));
    } catch {
